...
def format_clinic_facts(branch_id: str | None) -> str:
    ...
    return (
        ...
        f"You answer as {GROUP_TRADING_NAME}, the practice. Shellharbour (Barrack "
        "Heights), Dapto and Woonona are its three clinics. For a new booking, ask "
        "which clinic suits the caller before checking the diary, unless they have "
        "named a clinic, a suburb, or a dentist who works at only one. Help them "
        "choose by where they live, the dentist they want, their preference, or "
        "which clinic has something suitable soon. Run check_availability at that "
        "clinic (branch=its id, clinician=the dentist if they named one) before "
